...
const drawProjectiles = () => {
    ...
    ctx.fillStyle = "rgb(7, 250, 59)"; // Cor ciano para os projéteis do jogador
    playerProjectiles.forEach((projectile) => {
        ctx.fillRect(projectile.position.x, projectile.position.y, projectile.width, projectile.height);
        projectile.update();
    });

    // Desenha os projéteis dos invasores
    ctx.fillStyle = '#ff00ff'; // Cor magenta para os projéteis dos invasores
    invadersProjectiles.forEach((projectile) => {
        ctx.fillRect(projectile.position.x, projectile.position.y, projectile.width, projectile.height);
        projectile.update();
    });
};
